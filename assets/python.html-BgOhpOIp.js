...
<span class="line"></span></code></pre></div></details><br><h3 id="线程同步总结" tabindex="-1"><a class="header-anchor" href="#线程同步总结"><span>线程同步总结</span></a></h3><table><thead><tr><th>场景</th><th>需要的同步方式</th><th>说明</th></tr></thead><tbody><tr><td>主线程等待子线程完成</td><td><code>join()</code></td><td><strong>生命周期同步</strong>，等待线程结束</td></tr><tr><td>多线程修改共享变量</td><td><code>Lock / RLock</code></td><td><strong>互斥访问</strong>，防止数据竞争</td></tr><tr><td>限制并发访问数据库 / 网络 / GPU</td><td><code>Semaphore / BoundedSemaphore</code></td><td><strong>资源配额控制</strong>，限制同时访问数量</td></tr><tr><td>定时任务 / 启动信号 / 停止信号</td><td><code>Event</code></td><td><strong>状态通知</strong>，广播式唤醒</td></tr><tr><td>并行计算分阶段处理</td><td><code>Barrier</code></td><td><strong>阶段对齐</strong>，等待 n 个线程到达同一阶段点，再一起放行</td></tr><tr><td>生产者 — 消费者（手写队列）</td><td><code>Condition</code></td><td><strong>条件等待 + 精准唤醒</strong></td></tr><tr><td>生产者 — 消费者（标准写法）</td><td><code>Queue</code></td><td><strong>内置完成同步的安全队列</strong></td></tr><tr><td>线程池任务调度</td><td><code>Queue + Event</code></td><td><strong>任务流 + 停止通知</strong></td></tr><tr><td>多线程读多写少场景</td><td><code>RWLock</code> (第三方实现)</td><td><strong>读写分离锁</strong></td></tr><tr><td>多进程并发同步</td><td><code>multiprocessing</code> 同名同步对象</td><td><strong>跨进程同步</strong></td></tr></tbody></table><br><h3 id="线程和进程池" tabindex="-1"><a class="header-anchor" href="#线程和进程池"><span>线程和进程池</span></a></h3><div class="hint-container tip"><p class="hint-container-title">含义</p><p><code>concurrent.futures</code> = Python 的标准线程池 / 进程池任务调度框架</p><p><strong>它帮你解决：</strong> 线程创建与回收/任务队列/结果收集/异常传递/超时管理</p><p><strong>核心对象</strong></p><ul><li>ThreadPoolExecutor：线程池</li><li>ProcessPoolExecutor：进程池</li></ul></div><details class="hint-container details"><summary>ThreadPoolExecutor 实例方法</summary><p><strong>ThreadPoolExecutor 构造参数</strong></p><table><thead><tr><th>参数</th><th>说明</th></tr></thead><tbody><tr><td>max_workers</td><td>指定线程池的最大线程数，默认为CPU核数的5倍</td></tr><tr><td>thread_name_prefix</td><td>指定线程池中线程的名称前缀</td></tr></tbody></table><br><p><strong>ThreadPoolExecutor 实例方法</strong></p><table><thead><tr><th>方法</th><th>说明</th></tr></thead><tbody><tr><td>submit(fn, *args, **kwargs)</td><td>提交任务，线程池会分配一个线程迟总任务，返回一个Future实例<br>如果池已经满了，submit函数并不会阻塞，而是会将任务放到队列中，<br>查看源码发现使用的是queue.SimpleQueue()，简单队列，先进先出，队列大小没有限制</td></tr><tr><td>shutdown(wait=True)</td><td>清理池，池中的线程/进程全部杀掉，同时不再接受新提交的任务，<br>如果继续提交会报错<code>RuntimeError: cannot schedule new futures after shutdown</code></td></tr></tbody></table><br><p><strong>Future 实例方法</strong></p><table><thead><tr><th>方法</th><th>说明</th></tr></thead><tbody><tr><td>done()</td><td>如果调用成功执行或取消成功，那么返回True，否则返回False</td></tr><tr><td>cancelled()</td><td>如果复用被成功的取消，那么返回True</td></tr><tr><td>running()</td><td>如果正在运行且不能被取消，那么返回True</td></tr><tr><td>cancel()</td><td>尝试取消调用，如果已经执行且不能取消返回False，否则返回True</td></tr><tr><td>result(timeout=None)</td><td>阻塞执行，取返回的结果，timeout为None,一直等待返回，超时抛出concurrent.futures.TimeoutError异常<br>如果线程里的函数抛出了异常，<code>future.result()</code> 会在主线程里重新抛出同样的异常</td></tr><tr><td>add_done_callback(fn)</td><td>添加回调函数，无论任务成功还是异常，<strong>只要 Future 进入 done 状态</strong>，回调就会被调用一次</td></tr><tr><td>exception(timeout=None)</td><td>阻塞执行，取返回的异常，timeout为None,一直等待返回,超时抛出concurrent.futures.TimeoutError异常</td></tr></tbody></table></details><details class="hint-container details"><summary>add_done_callback 回调函数</summary><p><strong>基本概念：</strong></p><ul><li><p>只要 Future 进入 done 状态，这个回调都会被调用一次</p></li><li><p>如果注册多个回调，按注册顺序依次执行</p></li></ul><p><strong>特性</strong></p><ul><li>回调函数参数固定是 future 本身</li><li>如果submit提交的任务异常，回调仍然执行</li><li>如果submit提交的任务取消，回调仍然执行</li><li>回调执行在线程池的工作线程中，所以回调函数尽量快速执行，不要阻塞太久</li></ul></details><details class="hint-container details"><summary>什么时候该用 exception()</summary><p>1）add_done_callback 回调函数里</p><div class="language-python" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token keyword">def</span> <span class="token function">on_done</span><span class="token punctuation">(</span>f<span class="token punctuation">)</span><span class="token punctuation">:</span></span>
<span class="line">    <span class="token keyword">if</span> f<span class="token punctuation">.</span>exception<span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">:</span></span>
<span class="line">        log_error<span class="token punctuation">(</span>f<span class="token punctuation">.</span>exception<span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">)</span></span>
<span class="line"></span></code></pre></div><p>2）监控 / watchdog 线程</p><div class="language-python" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token keyword">for</span> f <span class="token keyword">in</span> futures<span class="token punctuation">:</span></span>
<span class="line">    <span class="token keyword">if</span> f<span class="token punctuation">.</span>done<span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token keyword">and</span> f<span class="token punctuation">.</span>exception<span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">:</span></span>
<span class="line">        restart_task<span class="token punctuation">(</span>f<span class="token punctuation">)</span></span>
<span class="line"></span></code></pre></div><p>3）GUI 主线程轮询后台任务</p><div class="language-python" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token keyword">if</span> future<span class="token punctuation">.</span>done<span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">:</span></span>
<span class="line">    err <span class="token operator">=</span> future<span class="token punctuation">.</span>exception<span class="token punctuation">(</span><span class="token punctuation">)</span></span>
<span class="line">    <span class="token keyword">if</span> err<span class="token punctuation">:</span></span>
<span class="line">        show_error_dialog<span class="token punctuation">(</span>err<span class="token punctuation">)</span></span>
<span class="line"></span></code></pre></div><p>4）统计任务失败率 / 批量汇总报告</p><div class="language-python" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line">failed <span class="token operator">=</span> <span class="token builtin">sum</span><span class="token punctuation">(</span><span class="token number">1</span> <span class="token keyword">for</span> f <span class="token keyword">in</span> futures <span class="token keyword">if</span> f<span class="token punctuation">.</span>exception<span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">)</span></span>
<span class="line"></span></code></pre></div><p>5）超时轮询型检查</p><div class="language-python" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token keyword">try</span><span class="token punctuation">:</span></span>
<span class="line">    e <span class="token operator">=</span> future<span class="token punctuation">.</span>exception<span class="token punctuation">(</span>timeout<span class="token operator">=</span><span class="token number">0.05</span><span class="token punctuation">)</span></span>
<span class="line"><span class="token keyword">except</span> TimeoutError<span class="token punctuation">:</span></span>
<span class="line">    <span class="token keyword">pass</span>   <span class="token comment"># 任务还在运行</span></span>
<span class="line"></span></code></pre></div></details><details class="hint-container details"><summary>线程池演示</summary><div class="language-python" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token comment">#!/usr/bin/env python</span></span>
<span class="line"><span class="token comment"># -*-coding:utf-8-*-</span></span>
<span class="line"></span>
<span class="line"><span class="token keyword">import</span> time</span>
<span class="line"><span class="token keyword">import</span> logging</span>
...
<span class="line"></span>
<span class="line"><span class="token comment"># 初始化日志</span></span>
<span class="line">FORMAT <span class="token operator">=</span> <span class="token string">&#39;%(asctime)-15s\\t [%(threadName)-13s] %(message)s&#39;</span></span>
<span class="line">logging<span class="token punctuation">.</span>basicConfig<span class="token punctuation">(</span>level<span class="token operator">=</span>logging<span class="token punctuation">.</span>INFO<span class="token punctuation">,</span> <span class="token builtin">format</span><span class="token operator">=</span>FORMAT<span class="token punctuation">)</span></span>
<span class="line"></span>
<span class="line"></span>
...
<span class="line"></span>
<span class="line">    <span class="token comment"># 模拟执行耗时</span></span>
...
<span class="line"></span>
<span class="line">    <span class="token comment"># 模拟意外报错</span></span>
<span class="line">    <span class="token keyword">if</span> x <span class="token operator">==</span> <span class="token number">2</span><span class="token punctuation">:</span></span>
...
<span class="line"></span>
<span class="line">    <span class="token comment"># 正确计算结果</span></span>
<span class="line">    ret <span class="token operator">=</span> x <span class="token operator">+</span> y</span>
<span class="line"></span>
...
<span class="line"></span>
<span class="line"></span>
<span class="line"><span class="token keyword">def</span> <span class="token function">on_done</span><span class="token punctuation">(</span>f<span class="token punctuation">)</span><span class="token punctuation">:</span></span>
<span class="line">    logging<span class="token punctuation">.</span>error<span class="token punctuation">(</span><span class="token string">&quot;回调函数: &quot;</span> <span class="token operator">+</span> <span class="token builtin">str</span><span class="token punctuation">(</span>f<span class="token punctuation">.</span>exception<span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">)</span></span>
<span class="line"></span>
<span class="line"></span>
...
<span class="line">    futures <span class="token operator">=</span> <span class="token punctuation">[</span>executor<span class="token punctuation">.</span>submit<span class="token punctuation">(</span>add<span class="token punctuation">,</span> i<span class="token punctuation">,</span> i<span class="token punctuation">)</span> <span class="token keyword">for</span> i <span class="token keyword">in</span> <span class="token builtin">range</span><span class="token punctuation">(</span><span class="token number">6</span><span class="token punctuation">)</span><span class="token punctuation">]</span></span>
<span class="line"></span>
<span class="line">    <span class="token comment"># 添加回调函数</span></span>
<span class="line">    <span class="token keyword">for</span> f <span class="token keyword">in</span> futures<span class="token punctuation">:</span></span>
<span class="line">        f<span class="token punctuation">.</span>add_done_callback<span class="token punctuation">(</span>on_done<span class="token punctuation">)</span></span>
<span class="line"></span>
...
<span class="line">            results<span class="token punctuation">.</span>append<span class="token punctuation">(</span>f<span class="token punctuation">.</span>result<span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">)</span></span>
...
<span class="line">            results<span class="token punctuation">.</span>append<span class="token punctuation">(</span>e<span class="token punctuation">)</span></span>
<span class="line"></span>
<span class="line">    <span class="token comment"># 统一处理</span></span>
...
<span class="line">        logging<span class="token punctuation">.</span>warning<span class="token punctuation">(</span><span class="token string-interpolation"><span class="token string">f&quot;add result: </span><span class="token interpolation"><span class="token punctuation">{</span>r<span class="token punctuation">}</span></span><span class="token string">&quot;</span></span><span class="token punctuation">)</span></span>
<span class="line"></span></code></pre></div></details><br><h3 id="queue-线程安全阻塞队列" tabindex="-1"><a class="header-anchor" href="#queue-线程安全阻塞队列"><span>queue - 线程安全阻塞队列</span></a></h3><div class="hint-container tip"><p class="hint-container-title">queue模块介绍</p><p><code>queue</code> 是 Python 标准库里的<strong>线程安全队列模块</strong>，主要用于<strong>多线程之间安全地传递数据</strong>，常见于<strong>生产者-消费者模型</strong></p><p><strong>核心对象</strong></p><table><thead><tr><th>对象</th><th>类型</th><th>取出顺序</th><th>典型用途</th></tr></thead><tbody><tr><td><code>queue.Queue</code></td><td>FIFO 队列</td><td>先进先出</td><td>生产者-消费者、任务分发</td></tr><tr><td><code>queue.LifoQueue</code></td><td>LIFO 队列</td><td>后进先出</td><td>线程安全栈、回溯任务</td></tr><tr><td><code>queue.PriorityQueue</code></td><td>优先级队列</td><td>按 priority 最小值先出</td><td>调度系统、重要任务优先</td></tr></tbody></table></div><details class="hint-container details"><summary>Queue类说明</summary><p><strong>初始化参数</strong></p><div class="language-python" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token keyword">def</span> <span class="token function">__init__</span><span class="token punctuation">(</span>self<span class="token punctuation">,</span> maxsize<span class="token operator">=</span><span class="token number">0</span><span class="token punctuation">)</span><span class="token punctuation">:</span></span>
<span class="line"></span></code></pre></div><ul><li>maxsize，队列最大容量，<code>0</code> 或负数代表 <strong>无限队列</strong></li></ul><br><p><strong>实例方法</strong></p><table><thead><tr><th>方法</th><th>作用</th><th>是否阻塞</th></tr></thead><tbody><tr><td><code>put(item, block=True, timeout=None)</code></td><td>放入元素</td><td>满了阻塞</td></tr><tr><td><code>get(block=True, timeout=None)</code></td><td>取出元素</td><td>空了阻塞</td></tr><tr><td><code>put_nowait(item)</code></td><td>非阻塞放入</td><td>等价 <code>put(block=False)</code></td></tr><tr><td><code>get_nowait()</code></td><td>非阻塞取出</td><td>等价 <code>get(block=False)</code></td></tr><tr><td><code>task_done()</code></td><td>标记一个任务已完成，<br>它配合 <code>q.join()</code> 实现任务完成等待机制</td><td>—</td></tr><tr><td><code>join()</code></td><td>等待所有任务完成</td><td>阻塞</td></tr><tr><td><code>qsize()</code></td><td>当前元素数量（近似值）</td><td>—</td></tr><tr><td><code>empty()</code></td><td>是否为空</td><td>—</td></tr><tr><td><code>full()</code></td><td>是否已满</td><td>—</td></tr><tr><td><code>shutdown()</code></td><td>关闭队列并唤醒所有阻塞线程</td><td>不阻塞</td></tr><tr><td><code>is_shutdown</code></td><td>判断队列是否已关闭</td><td>不阻塞</td></tr></tbody></table><br><p><strong>异常类</strong></p><table><thead><tr><th>异常</th><th>触发场景</th></tr></thead><tbody><tr><td><code>queue.Full</code></td><td><code>put</code> 超时或非阻塞队列满</td></tr><tr><td><code>queue.Empty</code></td><td><code>get</code> 超时或非阻塞队列空</td></tr><tr><td><code>queue.ShutDown</code></td><td>队列关闭后继续 put/get</td></tr></tbody></table><br><p><strong>当设置容量后</strong></p><table><thead><tr><th>操作</th><th>行为</th></tr></thead><tbody><tr><td><code>put()</code> 且队列满</td><td>阻塞等待直到有空位</td></tr><tr><td><code>put(block=False)</code></td><td>立刻抛 <code>queue.Full</code></td></tr><tr><td><code>get()</code> 且队列空</td><td>阻塞等待直到有元素</td></tr><tr><td><code>get(block=False)</code></td><td>立刻抛 <code>queue.Empty</code></td></tr></tbody></table></details><details class="hint-container details"><summary>生产者-消费者模型</summary><div class="language-python" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token keyword">import</span> time</span>
<span class="line"><span class="token keyword">import</span> random</span>
<span class="line"><span class="token keyword">import</span> threading</span>
<span class="line"><span class="token keyword">from</span> queue <span class="token keyword">import</span> Queue</span>
<span class="line"></span>
<span class="line">q <span class="token operator">=</span> Queue<span class="token punctuation">(</span>maxsize<span class="token operator">=</span><span class="token number">10</span><span class="token punctuation">)</span></span>
<span class="line"></span>
<span class="line"></span>
<span class="line"><span class="token keyword">def</span> <span class="token function">producer</span><span class="token punctuation">(</span>pid<span class="token punctuation">)</span><span class="token punctuation">:</span></span>
<span class="line">    <span class="token keyword">for</span> i <span class="token keyword">in</span> <span class="token builtin">range</span><span class="token punctuation">(</span><span class="token number">5</span><span class="token punctuation">)</span><span class="token punctuation">:</span></span>
<span class="line">        item <span class="token operator">=</span> <span class="token string-interpolation"><span class="token string">f&quot;P</span><span class="token interpolation"><span class="token punctuation">{</span>pid<span class="token punctuation">}</span></span><span class="token string">-task</span><span class="token interpolation"><span class="token punctuation">{</span>i<span class="token punctuation">}</span></span><span class="token string">&quot;</span></span></span>
...
<span class="line">        <span class="token keyword">print</span><span class="token punctuation">(</span><span class="token string-interpolation"><span class="token string">f&quot;[Producer </span><span class="token interpolation"><span class="token punctuation">{</span>pid<span class="token punctuation">}</span></span><span class="token string">] </span><span class="token interpolation"><span class="token punctuation">{</span>item<span class="token punctuation">}</span></span><span class="token string">&quot;</span></span><span class="token punctuation">)</span></span>
<span class="line">        time<span class="token punctuation">.</span>sleep<span class="token punctuation">(</span>random<span class="token punctuation">.</span>uniform<span class="token punctuation">(</span><span class="token number">0.1</span><span class="token punctuation">,</span> <span class="token number">0.4</span><span class="token punctuation">)</span><span class="token punctuation">)</span></span>
<span class="line"></span>
<span class="line"></span>
<span class="line"><span class="token keyword">def</span> <span class="token function">consumer</span><span class="token punctuation">(</span>cid<span class="token punctuation">)</span><span class="token punctuation">:</span></span>
<span class="line">    <span class="token keyword">while</span> <span class="token boolean">True</span><span class="token punctuation">:</span></span>
...
<span class="line">        <span class="token keyword">print</span><span class="token punctuation">(</span><span class="token string-interpolation"><span class="token string">f&quot;    [Consumer </span><span class="token interpolation"><span class="token punctuation">{</span>cid<span class="token punctuation">}</span></span><span class="token string">] </span><span class="token interpolation"><span class="token punctuation">{</span>item<span class="token punctuation">}</span></span><span class="token string">&quot;</span></span><span class="token punctuation">)</span></span>
<span class="line">        time<span class="token punctuation">.</span>sleep<span class="token punctuation">(</span>random<span class="token punctuation">.</span>uniform<span class="token punctuation">(</span><span class="token number">0.2</span><span class="token punctuation">,</span> <span class="token number">0.6</span><span class="token punctuation">)</span><span class="token punctuation">)</span></span>
...
<span class="line"></span>
<span class="line"></span>
<span class="line"><span class="token comment"># 启动 3 个消费者, 在本例子中，消费者线程是无限循环 + 阻塞等待，它永远不会自己结束, 所以设置 daemon = True, 随主线程退出</span></span>
...
<span class="line">    threading<span class="token punctuation">.</span>Thread<span class="token punctuation">(</span>target<span class="token operator">=</span>consumer<span class="token punctuation">,</span> args<span class="token operator">=</span><span class="token punctuation">(</span>i<span class="token punctuation">,</span><span class="token punctuation">)</span><span class="token punctuation">,</span> daemon<span class="token operator">=</span><span class="token boolean">True</span><span class="token punctuation">)</span><span class="token punctuation">.</span>start<span class="token punctuation">(</span><span class="token punctuation">)</span></span>
<span class="line"></span>
<span class="line"><span class="token comment"># 启动 2 个生产者</span></span>
<span class="line">producers <span class="token operator">=</span> <span class="token punctuation">[</span><span class="token punctuation">]</span></span>
<span class="line"><span class="token keyword">for</span> pid <span class="token keyword">in</span> <span class="token builtin">range</span><span class="token punctuation">(</span><span class="token number">2</span><span class="token punctuation">)</span><span class="token punctuation">:</span></span>
<span class="line">    t <span class="token operator">=</span> threading<span class="token punctuation">.</span>Thread<span class="token punctuation">(</span>target<span class="token operator">=</span>producer<span class="token punctuation">,</span> args<span class="token operator">=</span><span class="token punctuation">(</span>pid<span class="token punctuation">,</span><span class="token punctuation">)</span><span class="token punctuation">)</span></span>
<span class="line">    t<span class="token punctuation">.</span>start<span class="token punctuation">(</span><span class="token punctuation">)</span></span>
<span class="line">    producers<span class="token punctuation">.</span>append<span class="token punctuation">(</span>t<span class="token punctuation">)</span></span>
<span class="line"></span>
<span class="line"><span class="token comment"># 等待所有生产者结束</span></span>
<span class="line"><span class="token keyword">for</span> t <span class="token keyword">in</span> producers<span class="token punctuation">:</span></span>
<span class="line">    t<span class="token punctuation">.</span>join<span class="token punctuation">(</span><span class="token punctuation">)</span></span>
<span class="line"></span>
<span class="line"><span class="token comment"># 等待队列清空</span></span>
<span class="line">q<span class="token punctuation">.</span>join<span class="token punctuation">(</span><span class="token punctuation">)</span></span>
<span class="line"><span class="token keyword">print</span><span class="token punctuation">(</span><span class="token string">&quot;All tasks finished&quot;</span><span class="token punctuation">)</span></span>
<span class="line"></span></code></pre></div></details><details class="hint-container details"><summary><code>qsize()</code> / <code>empty()</code> / <code>full()</code> 在多线程中都不能作为可靠同步判断依据</summary><p><strong>哪些方法不可靠</strong></p><table><thead><tr><th>方法</th><th>问题</th></tr></thead><tbody><tr><td><code>qsize()</code></td><td>返回的是<strong>近似值</strong></td></tr><tr><td><code>empty()</code></td><td>可能刚判断完就被别的线程放入</td></tr><tr><td><code>full()</code></td><td>可能刚判断完就被别的线程取走</td></tr></tbody></table><p>原因很简单：</p><blockquote><p><strong>检查状态 ≠ 加锁后的真实状态</strong> 在你判断和下一行操作之间，别的线程可能已经改变队列。</p></blockquote><br><p>❌ <strong>错误方式</strong></p><div class="language-python" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token keyword">if</span> <span class="token keyword">not</span> q<span class="token punctuation">.</span>empty<span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">:</span></span>
<span class="line">    item <span class="token operator">=</span> q<span class="token punctuation">.</span>get<span class="token punctuation">(</span><span class="token punctuation">)</span>   <span class="token comment"># 这里仍可能阻塞！</span></span>
<span class="line"></span></code></pre></div><p>✅ <strong>正确方式</strong></p><p>直接依赖阻塞语义：</p><div class="language-python" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line">item <span class="token operator">=</span> q<span class="token punctuation">.</span>get<span class="token punctuation">(</span><span class="token punctuation">)</span>   <span class="token comment"># 自动等待直到安全取到</span></span>
<span class="line"></span></code></pre></div><p>或者：</p><div class="language-python" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token keyword">try</span><span class="token punctuation">:</span></span>
<span class="line">    item <span class="token operator">=</span> q<span class="token punctuation">.</span>get_nowait<span class="token punctuation">(</span><span class="token punctuation">)</span></span>
...
<span class="line"></span></code></pre></div><br><p><strong>那 qsize() 什么时候有用？</strong></p><p>✔ 仅用于 <strong>监控 / 日志 / UI 显示</strong> ✘ 不用于线程同步逻辑判断</p><p>比如打印进度：</p><div class="language-python" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token keyword">print</span><span class="token punctuation">(</span><span class="token string">&quot;Queue length:&quot;</span><span class="token punctuation">,</span> q<span class="token punctuation">.</span>qsize<span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">)</span></span>
<span class="line"></span></code></pre></div><p>即使不精确也没关系。</p></details><details class="hint-container details"><summary><code>PriorityQueue</code> 优先级队列</summary><p>PriorityQueue 是按优先级顺序出队的线程安全队列，内部基于 <code>heapq</code>（最小堆），但对外 API 与普通 <code>Queue</code> 完全一致</p><p>优先级在 put 时指定，而不是初始化时指定，数值越小 → 越先出队，比如</p><div class="language-python" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token keyword">from</span> queue <span class="token keyword">import</span> PriorityQueue</span>
<span class="line"></span>
<span class="line"><span class="token comment"># 优先级队列</span></span>
<span class="line">q <span class="token operator">=</span> PriorityQueue<span class="token punctuation">(</span>maxsize<span class="token operator">=</span><span class="token number">0</span><span class="token punctuation">)</span></span>
<span class="line"></span>
<span class="line"><span class="token comment"># 优先级设置方式</span></span>
<span class="line"><span class="token comment"># 	q.put((priority, item))</span></span>
<span class="line"><span class="token comment"># 		priority 数值越小 → 越先出队</span></span>
<span class="line"><span class="token comment"># 		item 为真正任务数据</span></span>
<span class="line">q<span class="token punctuation">.</span>put<span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token number">1</span><span class="token punctuation">,</span> <span class="token string">&quot;Hello World!&quot;</span><span class="token punctuation">)</span><span class="token punctuation">)</span></span>
<span class="line"></span>
<span class="line"><span class="token comment"># 注意事项</span></span>
<span class="line"><span class="token comment"># 当 priority 相等时，会比较第二个元素（item），如果item是不可比较的(比如字典), 就会报错</span></span>
<span class="line"></span></code></pre></div></details><br><h2 id="五、协程" tabindex="-1"><a class="header-anchor" href="#五、协程"><span>五、协程</span></a></h2><h3 id="同步和异步" tabindex="-1"><a class="header-anchor" href="#同步和异步"><span>同步和异步</span></a></h3><p>对于一个函数来说，同步指的是函数返回值就是我们想要的最终结果，异步则指不会直接拿到最终结果。</p><p>那么异步函数如何拿到最终结果，可以有如下几种方法</p><ul><li>给异步函数传入一个回调函数，在回调函数中我们可以拿到最终结果</li><li>异步函数返回给我们一个中间结果，比如说是一个对象，通过对象的xx方法可以拿到结果</li></ul><p><span style="color:red;font-weight:bold;">总结：同步/异步就看是否能直接拿到最终结果，需要注意的是与代码运行时长是没有任何关系的</span></p><br><h3 id="阻塞和非阻塞" tabindex="-1"><a class="header-anchor" href="#阻塞和非阻塞"><span>阻塞和非阻塞</span></a></h3><p>阻塞和非阻塞直接讲并不容易讲清楚，所以这里结合队列做一下说明</p><p>阻塞队列</p><ul><li>入队阻塞：当队列满了之后再入队，则会进行阻塞，直到满足以下情况： <ul><li>若设置了超时时间，入队超时，阻塞结束</li><li>当其他线程消费队列数据后导致队列不满，本线程入队成功，阻塞结束</li></ul></li><li>出队阻塞：当队列为空时再出队，则会进行阻塞，直到满足以下情况： <ul><li>若设置了超时时间，出队超时，阻塞结束</li><li>当其他线程入队数据后，本线程出队成功，阻塞结束</li></ul></li></ul><p>非阻塞队列</p><ul><li>入队不会阻塞，如何保证不会阻塞： <ul><li>不考虑内存限制的话，最简单的方法就是不设置队列大小，一直可以放入数据</li><li>如果考虑内存限制的话，可以使用双端队列，当队列满了之后再放入数据会先把最开始的数据删掉，然后就能正常放入</li></ul></li><li>出队不会阻塞，如何保证不会阻塞： <ul><li>当队列为空时返回<code>None</code>或者报错即可</li></ul></li></ul><p>阻塞队列-入队阻塞演示</p><details class="hint-container details"><summary>点击查看完整代码</summary><div class="language-python" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token comment">#!/usr/bin/env python</span></span>
<span class="line"><span class="token comment"># -*-coding:utf-8-*-</span></span>
<span class="line"></span>
...
<span class="line"></span></code></pre></div></details><h4 id="typing-callable" tabindex="-1"><a class="header-anchor" href="#typing-callable"><span><code>typing.Callable</code></span></a></h4><p>可调用类型，<code>Callable[[int], str]</code> 是把（int）转为 str 的函数</p>`,655)])])}const i=s(e,[["render",o]]),u=JSON.parse('{"path":"/programming/language/python.html","title":"Python","lang":"zh-CN","frontmatter":{},"git":{"updatedTime":1769039695000,"contributors":[{"name":"VVFock3r","username":"VVFock3r","email":"vvfock3r@gmail.com","commits":1,"url":"https://github.com/VVFock3r"}],"changelog":[{"hash":"cfa722a5c6ec950b4bd41a03f9061e61d63a8969","time":1769039695000,"email":"vvfock3r@gmail.com","author":"VVFock3r","message":"auto commit"}]},"filePathRelative":"programming/language/python.md"}');export{i as comp,u as data};
