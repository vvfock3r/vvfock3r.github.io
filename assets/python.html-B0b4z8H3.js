...
<span class="line"><span class="token comment"># End</span></span>
<span class="line"></span></code></pre></div></details><br><h3 id="线程局部变量" tabindex="-1"><a class="header-anchor" href="#线程局部变量"><span>线程局部变量</span></a></h3><p>创建一个全局变量<code>request = local()</code>，使用多线程对<code>request</code>进行修改时，会先将request拷贝到自身线程中一份，不会影响到全局和其他线程中的<code>request</code></p><details class="hint-container details"><summary>点击查看详情</summary><div class="language-python" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token comment">#!/usr/bin/env python</span></span>
<span class="line"><span class="token comment"># -*-coding:utf-8-*-</span></span>
<span class="line"></span>
<span class="line"><span class="token keyword">import</span> time</span>
<span class="line"><span class="token keyword">import</span> logging</span>
...
<span class="line">    request<span class="token punctuation">.</span>thread_name <span class="token operator">=</span> current_thread<span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">.</span>name</span>
<span class="line">    time<span class="token punctuation">.</span>sleep<span class="token punctuation">(</span><span class="token number">1</span><span class="token punctuation">)</span></span>
...
<span class="line"></span>
<span class="line"><span class="token keyword">print</span><span class="token punctuation">(</span><span class="token string">&quot;End&quot;</span><span class="token punctuation">)</span></span>
<span class="line"></span></code></pre></div><p>输出结果</p><div class="language-bash" data-highlighter="prismjs" data-ext="sh"><pre><code class="language-bash"><span class="line"><span class="token number">2026</span>-01-06 07:46:00,427	 <span class="token punctuation">[</span>Thread-1 <span class="token punctuation">(</span>thread_local<span class="token punctuation">)</span>, <span class="token number">7012</span><span class="token punctuation">]</span> request.thread_name: Thread-1 <span class="token punctuation">(</span>thread_local<span class="token punctuation">)</span></span>
<span class="line"><span class="token number">2026</span>-01-06 07:46:00,428	 <span class="token punctuation">[</span>Thread-4 <span class="token punctuation">(</span>thread_local<span class="token punctuation">)</span>, <span class="token number">12916</span><span class="token punctuation">]</span> request.thread_name: Thread-4 <span class="token punctuation">(</span>thread_local<span class="token punctuation">)</span></span>
<span class="line"><span class="token number">2026</span>-01-06 07:46:00,428	 <span class="token punctuation">[</span>Thread-6 <span class="token punctuation">(</span>thread_local<span class="token punctuation">)</span>, <span class="token number">10720</span><span class="token punctuation">]</span> request.thread_name: Thread-6 <span class="token punctuation">(</span>thread_local<span class="token punctuation">)</span></span>
<span class="line"><span class="token number">2026</span>-01-06 07:46:00,428	 <span class="token punctuation">[</span>Thread-5 <span class="token punctuation">(</span>thread_local<span class="token punctuation">)</span>, <span class="token number">9224</span><span class="token punctuation">]</span> request.thread_name: Thread-5 <span class="token punctuation">(</span>thread_local<span class="token punctuation">)</span></span>
<span class="line"><span class="token number">2026</span>-01-06 07:46:00,428	 <span class="token punctuation">[</span>Thread-2 <span class="token punctuation">(</span>thread_local<span class="token punctuation">)</span>, <span class="token number">11632</span><span class="token punctuation">]</span> request.thread_name: Thread-2 <span class="token punctuation">(</span>thread_local<span class="token punctuation">)</span></span>
<span class="line"><span class="token number">2026</span>-01-06 07:46:00,428	 <span class="token punctuation">[</span>Thread-3 <span class="token punctuation">(</span>thread_local<span class="token punctuation">)</span>, <span class="token number">12712</span><span class="token punctuation">]</span> request.thread_name: Thread-3 <span class="token punctuation">(</span>thread_local<span class="token punctuation">)</span></span>
<span class="line">End</span>
<span class="line"></span></code></pre></div></details><br><h3 id="线程同步-事件event" tabindex="-1"><a class="header-anchor" href="#线程同步-事件event"><span>线程同步 - 事件Event</span></a></h3><p>Event是线程通信间最简单的实现，使用一个变量flag，通过flag的True或False变化来执行不同操作。Event变化会通知到所有线程</p><p><strong>Event实例方法</strong></p><table><thead><tr><th>方法</th><th>说明</th></tr></thead><tbody><tr><td>set()</td><td>标记为True</td></tr><tr><td>clear()</td><td>标记为False</td></tr><tr><td>is_set()</td><td>新创建的event或使用<code>clear</code>()后为False，当设置<code>set</code>()后返回True</td></tr><tr><td>wait(timeout=None)</td><td>等待，满足以下条件后立即执行：<br>（1）当调用<code>set</code>()后立即执行，此时wait返回值为True<br>（2）当超时后立即执行，此时wait返回值为False<br>默认会一直等待，用不超时</td></tr></tbody></table><details class="hint-container details"><summary>点击查看详情</summary><div class="language-python" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token comment">#!/usr/bin/env python</span></span>
<span class="line"><span class="token comment"># -*-coding:utf-8-*-</span></span>
<span class="line"></span>
<span class="line"><span class="token keyword">import</span> time</span>
...
<span class="line"></span></code></pre></div></details><br><h3 id="线程同步-条件变量condition" tabindex="-1"><a class="header-anchor" href="#线程同步-条件变量condition"><span>线程同步 - 条件变量Condition</span></a></h3><div class="hint-container tip"><p class="hint-container-title">Condition</p><p><strong>设计思想</strong></p><p>让一个线程等某种条件成立，然后被另一个线程通知</p><br><p><strong>解释</strong></p><p>Condition 内部维护一把锁（默认 RLock），线程在持锁状态下可以调用 <code>notify()</code> 或 <code>notify_all()</code> 唤醒一个或多个等待线程。</p><p>调用 <code>wait()</code> 时，线程会释放锁并进入等待状态；</p><p>当被其他线程唤醒时，会重新获取锁并继续执行。</p><br><p><strong>注意事项</strong></p><ul><li>Condition 是底层工具，功能强大但容易写错，能用 <code>Queue</code> 或 <code>Event</code> 就不要用 Condition</li><li>Condition 不存储任何状态，所有状态变量（如 <code>turn</code>、buffer）必须在持锁期间访问和修改</li><li>先改变共享状态，再唤醒其他线程，避免丢信号或唤醒线程看到错误状态</li></ul></div><details class="hint-container details"><summary>Condition实例方法</summary><table><thead><tr><th>方法</th><th>说明</th></tr></thead><tbody><tr><td>acquire(blocking, timeout)</td><td>与RLock实例的acquire方法一致</td></tr><tr><td>release()</td><td>与RLock实例的release方法一致</td></tr><tr><td>notify(n=1)</td><td>唤醒一个或多个线程</td></tr><tr><td>notify_all()</td><td>唤醒所有线程</td></tr><tr><td>wait(timeout=None)</td><td>进入等待状态（这会释放锁），直到被唤醒或超时（又会重新获取锁），被唤醒返回True，超时返回False<br>如果未获得锁就使用wait，则报错<code>RuntimeError: cannot wait on un-acquired lock</code></td></tr><tr><td>wait_for(predicate, timeout=None)</td><td>wait_for是更灵活的一种wait方式，predicate需要指定一个可调用对象，<br>（1）当predicate()返回True，则继续往下执行<br>（2）当predicate()返回False，则和wait行为一致了<br>（3）timeout参数含义不变，若超时以后还会调用predicate一次</td></tr></tbody></table></details><details class="hint-container details"><summary>Condition举例：线程 A 打印数字 <code>1,2,3,...</code>，线程 B 打印字母 <code>A,B,C,...</code>，两个线程必须 <strong>交替打印</strong></summary><div class="language-python" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token keyword">from</span> threading <span class="token keyword">import</span> Thread<span class="token punctuation">,</span> Condition</span>
<span class="line"></span>
...
<span class="line"></span></code></pre></div></details><h4 id="typing-callable" tabindex="-1"><a class="header-anchor" href="#typing-callable"><span><code>typing.Callable</code></span></a></h4><p>可调用类型，<code>Callable[[int], str]</code> 是把（int）转为 str 的函数</p>`,617)])])}const i=s(e,[["render",o]]),u=JSON.parse('{"path":"/programming/language/python.html","title":"Python","lang":"zh-CN","frontmatter":{},"git":{"updatedTime":1767658964000,"contributors":[{"name":"VVFock3r","username":"VVFock3r","email":"vvfock3r@gmail.com","commits":1,"url":"https://github.com/VVFock3r"}],"changelog":[{"hash":"574772b2ba63e35dac20be9e37ee3b0c7ba37ea1","time":1767658964000,"email":"vvfock3r@gmail.com","author":"VVFock3r","message":"auto commit"}]},"filePathRelative":"programming/language/python.md"}');export{i as comp,u as data};
